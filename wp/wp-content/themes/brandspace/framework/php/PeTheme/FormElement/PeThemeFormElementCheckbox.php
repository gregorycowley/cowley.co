...
EOT;
	...
	protected function addTemplateMarkup(&$buffer,&$options,$value,$id,$name,$single) {
		...
		foreach ($options as $label=>$current) {
			...
			$buffer .=  '<div class="input_wrap"><input name="'.$name.'['.$count.']" id="'.$id.'_'.$count.'" type="checkbox" value="'.esc_attr($current).'"'.$selected.' /><label for="'.$name.'_'.$count.'">'.esc_attr($label).'</label></div>';
			$count++;
		}
	}
