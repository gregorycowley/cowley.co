...
class PeThemeGallery {
	...
	public function output($id = null) {
		...
		if ($loop) {
			...
			$this->master->get_template_part("gallery",empty($conf->type) ? "" : $conf->type);
		}
	}
		...
		foreach($post->images as $image) {
			...
			if (isset($image->data) ) {
				...
				if (is_array($image->data)) {
					...
					$data =& $image->data;
					if (!empty($data->caption_id)) {
						if (empty($data->caption)) {
							$id = function_exists("icl_object_id") ? icl_object_id($data->caption_id,"caption") : $data->caption_id;
							if ($id) {
								$caption = get_post($id); 
								if ($caption) {
									$data->caption = $caption->post_content;
								}
							}
						} else {
							unset($data->caption_id);
						}
					}
